...
class pload(object):
	...
	"""
        ...
        if ((not hasH5) and (datatype == 'hdf5')):
            print
            'To read AMR hdf5 files with python'
            print
            'Please install h5py (Python HDF5 Reader)'
            return
	...
	**Inputs**:
            ...
            try:
                ...
                if l.split()[0] == 'SCALARS':
                    ...
                    if (self.Slice):
                        ...
                        indxx = np.sort(
                            [n3_tot * n2_tot * k + j * n2_tot + i for i in self.irange for j in self.jrange for k in
                             self.krange])
                ...
                if ((ib > iend) or (ie < ibeg) or \
                    ...
                    for iv in range(nvar):
                        vars[ib0 - ibeg:ie0 - ibeg + 1, jb0 - jbeg:je0 - jbeg + 1, kb0 - kbeg:ke0 - kbeg + 1, iv] = \
                            myT.congrid(q1[:, :, :, iv].squeeze(), new_shape, method='linear', minusone=True).reshape(
    ...
    def DataScan(self, fp, n1, n2, n3, endian, dtype, off=None):
        ...
        if (self.Slice):
            darr = np.zeros((n1 * n2 * n3))
            indxx = np.sort(
                [n3_tot * n2_tot * k + j * n2_tot + i for i in self.irange for j in self.jrange for k in self.krange])
            if (sys.byteorder != self.endianess):
                A.byteswap()
            for ii, iii in enumerate(indxx):
                darr[ii] = A[iii]
	...
	"""
        ...
        print
        "Reading Data file : %s" % datafilename
	...
	**Inputs**:
	...
	"""
        ...
        if self.filetype == "single_file":
            ...
            print
            "Wrong file type : CHECK pluto.ini for file type."
            print
            "Only supported are .dbl, .flt, .vtk, .hdf5"
            sys.exit()
...
class Tools(object):
    ...
    def deriv(self, Y, X=None):
        ...
        **Inputs:**
        ...
        DfDx = np.roll(Yarr, 1) * (x12 / (x01 * x02)) + Yarr * (1. / x12 - 1. / x01) - np.roll(Yarr, -1) * (
            x01 / (x02 * x12))
        # Formulae for the first and last points:

        DfDx[0] = Yarr[0] * (x01[1] + x02[1]) / (x01[1] * x02[1]) - Yarr[1] * x02[1] / (x01[1] * x12[1]) + Yarr[2] * \
                                                                                                           x01[1] / (
                                                                                                               x02[1] *
                                                                                                               x12[
                                                                                                                   1])
        DfDx[n - 1] = -Yarr[n - 3] * x12[n2] / (x01[n2] * x02[n2]) + Yarr[n - 2] * x02[n2] / (x01[n2] * x12[n2]) - Yarr[
                                                                                                                       n - 1] * (
                                                                                                                       x02[
                                                                                                                           n2] +
                                                                                                                       x12[
...
class Image(object):
    ...
    def myfieldlines(self, Data, x0arr, y0arr, stream=False, **kwargs):
        ...
        **Usage:**
        ...
        if stream == True:
            ...
            contour(X, Y, StreamFunction, levels, colors=kwargs.get('colors'), linewidths=kwargs.get('lw', 1),
                    linestyles=kwargs.get('ls', 'solid'))
        else:
            for i in range(len(x0arr)):
                QxList.append(
                    self.field_line(Data.bx1, Data.bx2, Data.x1, Data.x2, Data.dx1, Data.dx1, x0arr[i], y0arr[i]).get(
                        'qx'))
                QyList.append(
                    self.field_line(Data.bx1, Data.bx2, Data.x1, Data.x2, Data.dx1, Data.dx1, x0arr[i], y0arr[i]).get(
                        'qy'))
    ...
    def getSphData(self, Data, w_dir=None, datatype=None, **kwargs):
	  ...
	  rphi -- [Default] is set to False implies that the r-theta plane is transformed. If set True then the r-phi plane is transformed.\n
        ...
        for variable in allvars:
            if kwargs.get('rphi', False) == True:
                R, Z, SphData[variable] = Tool.sph2cyl(Data, SphData.get(variable), rphi=True,
                                                       theta0=Data.x2[kwargs.get('x2cut', 0)])
            else:
                if Data.n3 != 1:
    ...
    def getPolarData(self, Data, ang_coord, rphi=False):
        ...
        **Outputs**:
        ...
        if ang_coord is D.x2:
            ...
            print
            "Angular co-ordinate must be given"
	...
	*Optional Keywords*:
        ...
        imshow(SphData[kwargs.get('plvar')], aspect='equal', origin='lower', cmap=cm.jet, extent=extent,
